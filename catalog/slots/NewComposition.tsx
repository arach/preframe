...
export function NewComposition({ initialMode }: { initialMode?: CompositionMode }) {
  ...
  }, [mode, name, musicPrompt, musicLyrics, lyricsResult, instrumental, engine, sources, prompt, refreshCatalog, setView]);
          ...
          <aside className="border-r border-white/[0.06] bg-white/[0.01] p-4 flex flex-col gap-3">
            ...
            {mode !== 'music' && (
              <div className="flex flex-col gap-2 pt-2 border-t border-white/[0.04]">
                <div className="text-[9px] font-mono uppercase tracking-[0.15em] text-white/20">Render Engine</div>
                <EngineButton engine="remotion" active={engine === 'remotion'} onClick={() => setEngine('remotion')} />
                <EngineButton engine="hyperframes" active={engine === 'hyperframes'} onClick={() => setEngine('hyperframes')} />
              </div>
            )}
          </aside>
...
function EngineButton({
  ...
  const detail = engine === 'remotion' ? 'React components → MP4' : 'HTML + GSAP → MP4';
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <div className={`text-[11px] font-mono ${active ? 'text-emerald-300/80' : 'text-white/50'}`}>{label}</div>
      <div className="text-[9px] font-mono text-white/28 mt-1 leading-snug">{detail}</div>
    </button>
  );
}
