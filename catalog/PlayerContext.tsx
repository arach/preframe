...
// ---------------------------------------------------------------------------
// Preframe-shape Media — a discriminated union so callers can pattern-match
...
type PreframeMeta = AudioAsset | Video;

function toMediaItem(m: Media): MediaItem<PreframeMeta> {
  if (m.kind === 'audio') {
    return {
      kind: 'audio',
      id: m.asset.id,
      ...
      artist: m.asset.app || 'Preframe',
      duration: m.asset.duration,
      artwork: [{ src: '/brand/preframe-logo.svg', sizes: 'any', type: 'image/svg+xml' }],
      meta: m.asset,
    };
  }
  ...
  return {
    ...
    artwork: frame && m.video.storyboardDir
      ...
      : [{ src: '/brand/preframe-logo.svg', sizes: 'any', type: 'image/svg+xml' }],
    meta: m.video,
  };
}

function fromMediaItem(item: MediaItem<PreframeMeta> | null): Media | null {
...
// ---------------------------------------------------------------------------
// Provider — wraps hudsonkit's PlayerProvider with the preframe-specific
...
function PlayerProviderInner({ children }: { children: ReactNode }) {
  ...
  const resolveItem = useCallback((kind: 'audio' | 'video', id: string): MediaItem<PreframeMeta> | null => {
    if (kind === 'audio') {
  ...
  return (
    <HudsonPlayerProvider
      appName="Preframe"
      persistenceKey="preframe:player"
...
// ---------------------------------------------------------------------------
// usePlayer — re-exports hudsonkit's state plus preframe-shape conveniences:
...
export function usePlayer() {
  ...
  const media = useMemo<Media | null>(
    () => fromMediaItem(hudson.media as MediaItem<PreframeMeta> | null),
    [hudson.media],
  );
  ...
  // History items in preframe shape. Audio entries need the full AudioAsset
  ...
  const queue = useMemo<Media[]>(
    () => (hudson.queue as MediaItem<PreframeMeta>[]).map(it => fromMediaItem(it)!).filter(Boolean),
    [hudson.queue],
  );
